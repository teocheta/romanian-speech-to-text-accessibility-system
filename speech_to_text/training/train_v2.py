...
# ------------ CONFIGURARE ALEGERE MODEL ------------
MODEL_TYPE = "cnn_lstm"  # alege "lstm" sau "cnn_lstm"
...
if MODEL_TYPE == "lstm":
    ...
    print("[INFO] Folosim modelul LSTM simplu.")
elif MODEL_TYPE == "cnn_lstm":
    model = CNNLSTMSpeechToText(input_dim=INPUT_DIM, hidden_dim=HIDDEN_DIM, output_dim=len(char2idx), num_layers=NUM_LAYERS)
    print("[INFO] Folosim modelul CNN + LSTM.")
else:
    raise ValueError("MODEL_TYPE trebuie sa fie 'lstm' sau 'cnn_lstm'")

# Încărcăm modelul dacă există deja și este compatibil
if os.path.exists("../model/model.pth"):
    try:
        model.load_state_dict(torch.load("../model/model.pth"))
        print("[INFO] Modelul existent a fost încărcat. Continuăm antrenarea...")
    except RuntimeError as e:
        print("[WARNING] Modelul existent nu este compatibil. Ștergem model.pth și începem de la zero.")
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, (inputs, transcripts) in enumerate(dataloader):
        ...
        input_lengths = torch.full((len(inputs),), inputs.size(2) // 4, dtype=torch.long)
...
print("Modelul final a fost salvat ca model.pth")
